...
namespace PWTS {
    struct PWTSHARED_EXPORT [[nodiscard]] FanData final {
        RWData<int> mode;
        QList<std::pair<int, int>> curve;

		friend QDataStream &operator<<(QDataStream &ds, const FanData &data) {
		    ds << data.mode <<
		    	data.curve;
		...
		friend QDataStream &operator>>(QDataStream &ds, FanData &data) {
			ds >> data.mode >>
				data.curve;
